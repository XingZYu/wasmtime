...
use wasmtime::{
    ...
    Table, TableType, Trap, Val, ValType, AdapterFunc,
};
...
const WASM_EXTERN_ADAPTER: wasm_externkind_t = 4;
...
pub unsafe extern "C" fn wasm_externtype_kind(et: *const wasm_externtype_t) -> wasm_externkind_t {
    match &(*et).ty {
        ...
        ExternType::Adapter(_) => WASM_EXTERN_ADAPTER,
    }
}
    ...
    fn from_val(val: &Val) -> wasm_val_t {
        ...
        match val {
            ...
            Val::String(s) => {
                ...
                wasm_val_t {
                    kind: from_valtype(&ValType::String),
    ...
    fn val(&self) -> Val {
        ...
        match into_valtype(self.kind) {
            ...
            ValType::String => Val::from(unsafe { 
                CStr::from_ptr(self.of.string).to_str().unwrap() 
            }),
...
pub unsafe extern "C" fn wasm_val_vec_new(
    ...
    size: usize,
    source: *const wasm_val_t,
) {
    let slice = slice::from_raw_parts(source, size);
    (*out).set_from_slice(slice);
}

#[no_mangle]
pub unsafe extern "C" fn wasm_val_vec_new_uninitialized(out: *mut wasm_val_vec_t, size: usize) {
    (*out).set_uninitialized(size);
}
...
impl wasm_adapter_t {
    fn adapter(&self) -> &HostRef<AdapterFunc> {
        match &self.ext.which {
            ExternHost::Adapter(f) => f,
            _ => unsafe { std::hint::unreachable_unchecked() },
        }
    }
}
...
#[no_mangle]
pub unsafe extern "C" fn wasm_adapter_call(
    func: *const wasm_adapter_t,
    args: *const wasm_val_t,
    results: *mut wasm_val_t,
) -> *mut wasm_trap_t {
    let func = (*func).adapter().borrow();
...
#[no_mangle]
pub unsafe extern "C" fn wasm_adapter_delete(f: *mut wasm_adapter_t) {
...
pub unsafe extern "C" fn wasm_extern_kind(e: *const wasm_extern_t) -> wasm_externkind_t {
    match (*e).which {
        ...
        ExternHost::Adapter(_) => WASM_EXTERN_ADAPTER,
    }
}
...
pub unsafe extern "C" fn wasm_extern_type(e: *const wasm_extern_t) -> *mut wasm_externtype_t {
    let et = Box::new(wasm_externtype_t {
        ty: match &(*e).which {
            ...
            ExternHost::Adapter(f) => ExternType::Adapter(f.borrow().ty().clone()),
        },
        cache: wasm_externtype_t_type_cache::Empty,
    });
    Box::into_raw(et)
...
#[no_mangle]
pub unsafe extern "C" fn wasm_extern_as_adapter(e: *mut wasm_extern_t) -> *mut wasm_adapter_t {
    match &(*e).which {
        ExternHost::Adapter(_) => e.cast(),
        _ => ptr::null_mut(),
    }
}
...
pub unsafe extern "C" fn wasm_instance_new(
    ...
    for i in 0..(*module).imports.len() {
        ...
        externs.push(match &(*import).which {
            ...
            ExternHost::Adapter(e) => Extern::Adapter(e.borrow().clone()),
        });
    }
    let store = &(*store).store.borrow();
    ...
    let exports = cache.get_or_insert_with(|| {
        ...
        instance
            ...
            .map(|e| match e {
                ...
                Extern::Adapter(f) => ExternHost::Adapter(HostRef::new(f.clone())),
            })
            .collect()
    });
...
#[no_mangle]
pub unsafe extern "C" fn wasm_get_export(
    instance: *const wasm_instance_t,
    name: *const wasm_name_t,
    result: *mut wasm_extern_vec_t
) -> *mut wasm_trap_t {
    let instance = &(*instance).instance.borrow();
    let u8_name = (*name).as_slice();
    let string_name = String::from_utf8(u8_name.to_vec()).unwrap();
    let export = match instance.get_export(&string_name) {
        Some(s) => s,
        None => {
            let trap = Box::new(
                wasm_trap_t { trap: HostRef::new(Trap::new("Export Not Found")) }
            );
